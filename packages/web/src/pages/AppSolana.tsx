...
import { Loader2, Copy, ExternalLink, X, Search, Wallet, Receipt, Image, Shield, Layers, Activity, TrendingUp } from 'lucide-react';
            ...
            <div className="solana-shell">
                ...
                {portfolio && !loading && (
                    <div className="solana-content">
                        <div className="solana-wallet-header">
                            ...
                            <div className="total-value">
                                ...
                                <span className="total-amount">{formatUsd(portfolio.totalUsd)}</span>
                            </div>
                        </div>

                        <div className="solana-tabs">
                            {tabs.map(tab => (
                                <button
                                    key={tab.id}
                        ...
                        <div className="tab-panel">
                            {activeTab === 'portfolio' && (
                                <div className="portfolio-panel">
                                    <div className="sol-balance-card">
                                        <div className="sol-icon">
                                            <span>◎</span>
                                        </div>
                                        <div className="sol-info">
                                            <span className="sol-amount">{formatSol(portfolio.sol.lamports)} SOL</span>
                                            <span className="sol-usd">{formatUsd(portfolio.sol.usd)}</span>
                                        </div>
                                        {portfolio.staking && portfolio.staking.length > 0 && (
                                            <div className="staking-info">
                                                <span className="staking-label">Staked</span>
                                                <span className="staking-value">{portfolio.staking.length} accounts</span>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    {portfolio.tokens && portfolio.tokens.length > 0 && (
                                        <div className="tokens-panel">
                                            <h3>Token Holdings ({portfolio.tokens.length})</h3>
                                    ...
                                    {transactions.length > 0 ? (
                                        <div className="txs-list">
                                            {transactions.map((tx, idx) => (
                                                <div key={idx} className={`tx-item ${tx.status}`}>
                                                    <div className="tx-icon">
                                                        {tx.amount ? (
                                                            tx.amount > 0 ? <TrendingUp size={14} /> : <Activity size={14} />
                                                        ) : <Activity size={14} />}
                                                    </div>
                                                    ...
                                                    <div className="tx-amount">
                                                        {tx.amount && (
                                                            <span className={tx.amount > 0 ? 'received' : 'sent'}>
                                    ...
                                    {risk.signals.length > 0 && (
                                        <div className="risk-signals">
                                            ...
                                            {risk.signals.map((signal, idx) => (
                                                <div key={idx} className={`signal-item ${signal.severity}`}>
                                                    <Shield size={14} />
                                                    <span className="signal-name">{signal.name}</span>
                                                    <span className="signal-badge">{signal.severity}</span>
                                                </div>
                                            ))}
                                        </div>
                                    )}
