...
interface GraphNode extends d3.SimulationNodeDatum {
  ...
  address: string;
  label: string;
  type: 'wallet' | 'contract' | 'exchange' | 'mixer' | 'dao' | 'nft' | 'defi' | 'target' | 'oracle' | 'bridge' | 'transaction';
  balance?: number;
...
const AdvancedGraph: React.FC<{ targetAddress?: string; chain?: string; onClose?: () => void }> = ({ targetAddress, chain = 'ethereum', onClose }) => {
  ...
  // Auto-generate graph when targetAddress is provided
  const hasAutoGenerated = useRef(false);
  useEffect(() => {
    if (targetAddress && !isGenerated && !isLoading && !hasAutoGenerated.current) {
      hasAutoGenerated.current = true;
      setWalletAddress(targetAddress);
      setIsMerged(false);
      handleGenerate();
    }
  }, [targetAddress, isGenerated, isLoading, handleGenerate]);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      const response = await analyzeWallet(addressToUse, chain as any, { limit: 100000 });
      ...
      notify.info(`Found ${txns.length} transactions, building graph with ${showTransactionNodes ? 'transaction nodes' : 'address-only'}...`);

      const nodes: GraphNode[] = [];
      const edges: GraphEdge[] = [];
      const addressMap = new Map<string, GraphNode>();
      const txNodeMap = new Map<string, GraphNode>();
      ...
      // Sort transactions by timestamp for timeline view
      ...
      sortedTxns.forEach((tx: any, idx: number) => {
        ...
        const txHash = tx.hash || tx.tx_hash || tx.transactionHash;
        ...
        // Add sender node (only if not already added)
        ...
        // Add receiver node (only if not already added)
        ...
        // If showing transaction nodes, create them
        if (showTransactionNodes) {
          const txNodeId = `tx_${idx}`;
          
          // Create transaction node with all tx data
          const txNode: GraphNode = {
            id: txNodeId,
            address: txHash || txNodeId,
            label: `${txValue.toFixed(4)} ETH`,
            type: 'transaction',
            isTransactionNode: true,
            txHash,
            txValue,
            txToken: tx.token || 'ETH',
            txTimestamp: txTime,
            txStatus: tx.error || tx.failed ? 'failed' : 'success',
            txFrom: fromAddr,
            txTo: toAddr,
            txMethod: tx.method || tx.functionName || tx.methodId || '',
            txGasUsed: tx.gasUsed || tx.gas,
          };
          
          nodes.push(txNode);
          txNodeMap.set(txNodeId, txNode);

          // Add edge from sender to transaction node
          edges.push({
            id: `edge_sent_${idx}`,
          ...
          // Add edge from transaction node to receiver
          edges.push({
            id: `edge_recv_${idx}`,
            source: txNode,
          ...
          // Original behavior - direct edge between addresses
      ...
      if (targetNode) {
        ...
        const sortedWithTime = sortedTxns.filter((t: any) => t.timestamp);
        if (sortedWithTime.length > 0) {
          targetNode.firstTx = new Date(sortedWithTime[0].timestamp).getTime();
          targetNode.lastTx = new Date(sortedWithTime[sortedWithTime.length - 1].timestamp).getTime();
        }
      }
  ...
  }, [walletAddress, targetAddress, chain, notify, graphData, performAIAnalysis, showTransactionNodes]);
    ...
    <div className={`advanced-graph ${themeMode === 'light' ? 'theme-light' : 'theme-dark'} ${presentationMode === 'presentation' ? 'presentation-mode' : ''} ${isStealth ? 'stealth-mode' : ''}`}>
      ...
      <div ref={containerRef} className="graph-container" style={{ display: isGenerated && !isLoading ? 'block' : 'none' }}>
        ...
        <div className="graph-controls-top">
          ...
          {(targetAddress || walletAddress) && isGenerated && (
            <>
              <button 
          ...
          <div className="graph-details-panel">
            <div className="panel-header">
              <div className="panel-title">
                {selectedNode.isTransactionNode ? (
                  <>
                    <span className="node-indicator transaction" />
                ...
                <>
                  <div className="detail-row">
                    ...
                    <div className="detail-value-row">
                      <code>{selectedNode.txHash?.slice(0, 10)}...{selectedNode.txHash?.slice(-8)}</code>
                      <button className="icon-btn" onClick={() => selectedNode.txHash && handleCopyAddress(selectedNode.txHash)} title="Copy"><Icon name="copy" size={14} /></button>
                    </div>
                  </div>
                  <div className="detail-row">
                    <span className="detail-label">Value</span>
                    <span className="detail-value">{selectedNode.txValue?.toFixed(6)} {selectedNode.txToken || 'ETH'}</span>
                  </div>
                  <div className="detail-row">
                    <span className="detail-label">Status</span>
                    <span className={`tx-status ${selectedNode.txStatus}`}>{selectedNode.txStatus}</span>
                  </div>
                  <div className="detail-row">
                    <span className="detail-label">Timestamp</span>
                    <span className="detail-value">{selectedNode.txTimestamp ? new Date(selectedNode.txTimestamp).toLocaleString() : 'N/A'}</span>
                  </div>
                  <div className="detail-row">
                    <span className="detail-label">From</span>
                    <div className="detail-value-row">
                      <code>{selectedNode.txFrom?.slice(0, 10)}...{selectedNode.txFrom?.slice(-8)}</code>
                      <button className="icon-btn" onClick={() => selectedNode.txFrom && handleCopyAddress(selectedNode.txFrom)} title="Copy"><Icon name="copy" size={14} /></button>
                    </div>
                  </div>
                  <div className="detail-row">
                    <span className="detail-label">To</span>
                    <div className="detail-value-row">
                      <code>{selectedNode.txTo?.slice(0, 10)}...{selectedNode.txTo?.slice(-8)}</code>
                      <button className="icon-btn" onClick={() => selectedNode.txTo && handleCopyAddress(selectedNode.txTo)} title="Copy"><Icon name="copy" size={14} /></button>
                    </div>
                  </div>
                  {selectedNode.txMethod && (
                    <div className="detail-row">
                      <span className="detail-label">Method</span>
                      <span className="detail-value method">{selectedNode.txMethod}</span>
                    </div>
                  )}
                  {selectedNode.txGasUsed && (
                    <div className="detail-row">
                      <span className="detail-label">Gas Used</span>
                      <span className="detail-value">{Number(selectedNode.txGasUsed).toLocaleString()}</span>
                    </div>
                  )}
                  <div className="panel-actions">
                    <button className="action-btn" onClick={() => selectedNode.txHash && window.open(`https://etherscan.io/tx/${selectedNode.txHash}`, '_blank')}><Icon name="externalLink" size={16} />View on Explorer</button>
                    <button className="action-btn" onClick={() => selectedNode.txHash && handleCopyAddress(selectedNode.txHash)}><Icon name="copy" size={16} />Copy Hash</button>
                  </div>
                </>
              ) : (
                <>
