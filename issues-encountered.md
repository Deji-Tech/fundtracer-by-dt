...
4. **Model-aware wait times** - Larger models (4B, 8B) get longer wait windows (up to 10 minutes for 8B)
...
### Solution
- Increased retry attempts from 15 to 200+ (depending on model size)
- Added console message showing expected wait time
- Improved error handling and logging
- Added check to verify model is actually loaded (not just port open)
...
## qvac stop Command Not Working

### Problem
When running `fundtracer qvac stop`, the CLI would show QVAC status instead of stopping the server.

### Root Cause
Commander.js handles default actions with subcommands differently. The `.action()` method for the parent command was overriding the subcommand actions.

### Solution
Moved the default action (status check) after defining all subcommands in the code:
```typescript
const qvacCmd = program.command('qvac');
qvacCmd.command('status').action(statusHandler);
qvacCmd.command('stop').action(stopHandler);
// Default action LAST
qvacCmd.action(defaultHandler);
```
...
1. **Large model load times**: 4B and 8B models can take 3-10 minutes to load on first run
